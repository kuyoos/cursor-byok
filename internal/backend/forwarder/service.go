...
func extractConversationActionPrependUserMessages(action *agentv1.ConversationAction) []*agentv1.UserMessage {
	...
	item, ok := action.GetAction().(*agentv1.ConversationAction_UserMessageAction)
	if !ok || item.UserMessageAction == nil {
		return nil
	}
	field := item.UserMessageAction.ProtoReflect().Descriptor().Fields().ByName("prepend_user_messages")
	if field == nil {
		return nil
	}
	values := item.UserMessageAction.ProtoReflect().Get(field).List()
	messages := make([]*agentv1.UserMessage, 0, values.Len())
	for index := 0; index < values.Len(); index++ {
		message, ok := values.Get(index).Message().Interface().(*agentv1.UserMessage)
		if ok && message != nil {
